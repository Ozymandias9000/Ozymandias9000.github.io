    ...
    <div class="container relative h-screen">
      <div class="fixed top-2 right-4 z-50">
        ...
        <button
          ...
          class="prose pr-8 cursor-pointer transform transition-colors duration-2000 {checked ? 'text-white hover:text-gray-400' : ''}">What
          I Do</button>
        <button
          on:click={() => scrollContainer.scrollTo({ top: scrollContainer.scrollHeight, left: 0, behavior: 'smooth' })}
          class="prose pr-8 cursor-pointer transform transition-colors duration-2000 {checked ? 'text-white hover:text-gray-400' : ''}">Contact
          Me</button>
      </div>
      ...
      <div class="flex flex-col justify-center items-center h-3/5">
        <IntersectionObs let:intersecting top={0}>
          ...
          {#if !intersecting}
            <div
              class="fixed top-0 right-0 transform transition-colors duration-2000 {checked ? 'bg-pacific-700' : 'bg-pacific-100'}  opacity-95 h-12 w-full pointer-events-none z-40" />
